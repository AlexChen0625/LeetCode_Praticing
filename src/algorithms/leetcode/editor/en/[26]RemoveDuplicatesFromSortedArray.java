...
// 
// Related Topics Array Two Pointers 👍 5211 👎 8664
...
class Solution {
	public int removeDuplicates(int[] nums) {
		int nonDuplicatesNum = 0;
		for (int i = 1; i < nums.length; i++) {
			if (nums[i] != nums[nonDuplicatesNum]) {
				nums[nonDuplicatesNum] = nums[i];
				nonDuplicatesNum += 1;

			}
		}
		System.out.println("nonDuplicatesNum: " + nonDuplicatesNum);
		return nonDuplicatesNum;
	}
}
